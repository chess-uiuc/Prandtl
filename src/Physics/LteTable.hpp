...
namespace Prandtl
{
  ...
  inline void uniform_grid(int N, double min, double max, mfem::Vector &grid)
  {
    grid.SetSize(N);
  ...
  inline void log_grid(int N, double min, double max, mfem::Vector &grid)
  {
    grid.SetSize(N);
                             ...
                             real_t* lte_table, real_t &e_min, real_t &e_max, MPI_Comm comm)
  {
    ...
    real_t UKB = 1.380649e-23; // Boltzmann constant in J/K
    ...
    real_t R, P, nb, e, betaT, alpha, cv, gam, cp, c,
           mu, sigma, lambda_trh, lambda_tre, lambda_int, lambda_reactive;
    ...
    for(int k=k0; k < k1; k++)
    {
      ...
      plato_get_species_energy(&T, ei.GetData());
      ...
      plato_get_transp_coeff_comp(&nb, Xi.GetData(), &T, &mu, &sigma, &lambda_trh, &lambda_tre, &lambda_int, Di.GetData());
      ...
      double X_tol = 1e-14;
      plato_comp_tol(&X_tol, Xi.GetData(), Xitol.GetData());
      plato_get_species_diff_flux(&T, &T, &nb, Xitol.GetData(), Dij.GetData(), di.GetData(), Ji.GetData());
      double lambda_reactive = dot_product(Ji.GetData(), hi.GetData(), nb_spec);
      ...
      lte_table[L.lte_property_index(L.lambda_idx, i, j)] = lambda_trh + lambda_tre + lambda_int + lambda_reactive;
