...
TEST(plato_library_init_test)
{
    ...
    std::string path    = "/Users/neo/Workspace/sources/codes/database";
...
TEST(plato_Temperature_solve_test)
{
    ...
    real_t T   = 0.768256*T_grid[ny/12] + 0.231744*T_grid[4*ny/5];
    ...
    real_t T_inv_table = eos.biinterp_inverse_table(*phys, L, S1);
    real_t T_newton    = eos.temp_from_internal_energy(*phys, L, S1);

    real_t P_true = mix.P;
    real_t P_tab_direct  = eos.biinterp_lte_table(L.P_idx,*phys, L, S1, T);
    real_t P_tab_newton  = eos.pressure(*phys, L, S1);
    real_t e_newton = eos.biinterp_lte_table(L.e_idx,*phys, L, S1, T_newton);

    EXPECT_CLOSE(e/e, e_newton/e, 1e-14);

    std::cout << "\n";
    std::cout << "T_inv_table - T_true = " << T_inv_table - T << std::endl;
    std::cout << "T_newton - T_true = " << T_newton - T << std::endl;
    ...
    real_t rhoe_inverse = eos.internal_energy_from_pressure(*phys, L, S_random, P_true);
...
TEST(plato_Tablelookup_test)
{
    ...
    for(real_t rho_true = rho_grid[3]; rho_true <= rho_grid[4] ; rho_true += 0.0001)
    {
        ...
        real_t P_table = eos.pressure(*phys, L, S4);
        ...
        real_t rhoe_inverse = eos.internal_energy_from_pressure(*phys, L, S5, P_table);
