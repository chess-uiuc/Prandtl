...
plato_set_state(const real_t rho, const real_t T, PlatoMixture &mix)
{
    ...
    R  = Prandtl::dot_product(Ri.GetData(), yi.GetData(), nb_spec);
    P  = rho * R * T;
    nb = P / (UKB*T);
    ...
    e = Prandtl::dot_product(yi.GetData(), ei.GetData(), nb_spec);
    ...
    lambda_reactive = Prandtl::dot_product(Ji.GetData(), hi.GetData(), nb_spec);
